...
## 予想エンジン（アンサンブル・費用ゼロで高精度）

複数の無料手法を統合: 特徴量 → 期待得点(λ) → **Poisson / Dixon-Coles** → 1X2確率。

- **Dixon-Coles 最尤推定**（`src/lib/dixon-coles.ts`）… 過去全結果からチームの攻撃力/守備力・
  ホーム優位・低スコア補正(ρ)・得点水準を**時間減衰つきで最尤推定**（勾配上昇＋L2縮約、
  水準は閉形式で厳密再推定、ρは1次元探索）。サッカー1X2予測の最高峰手法。実データ取込時に有効化し、
  Eloベースの見積りと重み付きでアンサンブル。合成データでの係数回復を検証済み（att相関≈0.98）。
- **Eloレーティング**（`src/lib/elo.ts`）… 実力。ホームに+65点相当。
- **攻撃力・守備力・直近5試合のフォーム**… 平均得失点と調子で微調整。
- **群衆の投票率（支持率）を事前分布化**… toto公式の支持率をベイズ的prior/アンサンブルに統合。
  データの無いJ2/J3対戦では群衆を重視、実データのある対戦ではモデルを重視（データ信頼度で自動配分）。
- **Dixon-Coles補正**（`src/lib/poisson.ts`）… 低スコア・引き分けの多さを補正。
- **自信度** = 最有力結果の確率、**波乱度** = 1X2分布の正規化エントロピー。

### 妙味(value)分析・精密評価

- **妙味(value)**: モデル確率と群衆支持率の差（エッジ）を各試合に表示（過小評価＝狙い目）。
- **評価指標**（`src/lib/metrics.ts`）: RPS（順序考慮・1X2の標準）/ Brier / 対数損失 / 情報利得 /
  キャリブレーション。成績検証ページでモデル比較。
- **的中数の厳密分布**（`src/lib/simulate.ts`）: ポアソン二項分布の畳み込みで本命の期待的中数や
  買い目のP(≥k)を厳密計算（モンテカルロより正確）。買い目ページに表示。
